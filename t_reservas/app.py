...
def cargar_datos():
    ...
    for item in response['Items']:
        ...
        seats_reserved = item['seats_reserved']['N']
        reservation_date = item['reservation_date']['S']
        status = item['status']['S']
        data.append([cinema_id, user_id, show_id, seats_reserved, reservation_date, status])
    return data
...
def guardar_en_s3(data, filename):
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow(["cinema_id", "user_id", "show_id", "seats_reserved", "reservation_date", "status"])
...
def ingesta_datos():
    data = cargar_datos()
    guardar_en_s3(data, 't_reservas_data.csv')
